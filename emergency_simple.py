...
def emergency_fix_tiles_simple(file_path):
    """Emergency fix - add CSS directly with !important flags - simple version"""
    ...
    try:
        ...
        if '<head>' in content:
            ...
            with open(file_path, 'w', encoding='utf-8') as file:
                file.write(content)
            print(f"SUCCESS: Emergency fix applied to {os.path.basename(file_path)}")
            return True
        else:
            print(f"FAILED: No <head> tag found in {os.path.basename(file_path)}")
            return False
...
def main():
    ...
    for file in files:
        if os.path.exists(file):
            emergency_fix_tiles_simple(file)
        else:
