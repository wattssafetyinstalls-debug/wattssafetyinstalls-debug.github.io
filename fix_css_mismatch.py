#!/usr/bin/env python3
"""
Replace service page CSS with CSS that matches their simple HTML structure
"""

import os

# Simple CSS that matches the service page template structure
SIMPLE_CSS = """
        :root {
            --teal: #00C4B4;
            --navy: #0A1D37;
...
def fix_css_mismatch():
    """Replace service page CSS with CSS that matches their HTML structure"""
    ...
    for filename in os.listdir(services_dir):
        if filename.endswith('.html') and not filename.endswith('.backup'):
            ...
            # Replace the entire style section with simple CSS
            if '<style>' in content and '</style>' in content:
                start = content.find('<style>')
                end = content.find('</style>') + 8
                new_content = content[:start] + '<style>' + SIMPLE_CSS + '</style>' + content[end:]
                
                with open(filepath, 'w', encoding='utf-8') as f:
                    f.write(new_content)
                
                fixed_count += 1
                print(f"Fixed CSS mismatch: {filename}")
    
    print(f"Fixed CSS mismatch in {fixed_count} service pages")
